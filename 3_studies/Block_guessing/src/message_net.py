...
class MessageNet(torch.nn.Module):
    """
    A flexible MLP to compute messages m_{u→v} from [h_u || h_v || e_{u→v}].
    ...
    Args:
      ...
      num_layers (int): total number of Linear→ReLU blocks. The final block
                        produces the message embedding of size hidden_dim.
                        Minimum = 1 (just one Linear→ReLU from input_dim→hidden_dim).
    ...
    def __init__(self, input_dim, hidden_dim, num_layers=2, dropout=0.0):
        ...
        # Add (num_layers - 1) hidden blocks: Linear→ReLU→(Dropout if set)
        ...
        # Final block: Linear→ReLU to produce a hidden_dim‐sized message
        layers.append(torch.nn.Linear(in_dim, hidden_dim))
        layers.append(torch.nn.ReLU())
    ...
    def forward(self, x):
        """
        x: Tensor of shape (E_total, input_dim), e.g. concatenated [h_u, h_v, e_uv]
