...
\begin{table}[H]
    \centering
    \caption[GNN on QM9 full dataset sample with full matrix loss]{GNN on QM9 full dataset sample using full matrix loss to train. Metrics on test set and corresponding hyperparameter settings for 10 best performing networks in terms of iterations.}
...
\begin{table}[H]
    \centering
    \caption[Comparison of different models employing \textsc{PySCF} and $\overline{P}$ guessing schemes on the QM9 full dataset.]{Comparison of different models employing \textsc{PySCF} and $\overline{P}$ guessing schemes on the QM9 full dataset. Here, $\overline{P}$ is computed block-wise across all molecules by averaging over each \ch{C}, \ch{O}-\ch{H}, \dots block.
}
