import psi4
import os

os.chdir(os.path.dirname(os.path.abspath(__file__)))

def read_molecule(path: str) -> (psi4.core.Molecule, float):
    with open(path, "r") as fin:
        lines = fin.readlines()
    # read charge and multiplicity from the comment line and
    # replace the comment line with just these two integers
    split = lines[1].split()
    q = int(split[split.index("charge") + 1])
    s = int(split[split.index("multiplicity") + 1])
    lines[1] = f"{q} {s}\n"
    xyz_string = "".join(lines)
    mol = psi4.core.Molecule.from_string(xyz_string, dtype="xyz+")

    E0 = float(split[12])
    ZPE = float(split[11])

    return mol, E0 - ZPE


def main():
    psi4.set_memory("4 GiB")
    psi4.set_options(
        {
            "BASIS": "custom_basis",
            "SCF_TYPE": "PK",  # Disable density fitting
            "dft_spherical_points": 590,
            "dft_radial_points": 99,
        }
    )

    mol, e_ref = read_molecule("./064590_valid.xyz")

    e, final_wfn = psi4.energy(
        name="B3LYP",
        molecule=mol,
        return_wfn=True,
    )
    print(f"Final energy: {e} Ha compared to the reference energy of {e_ref}")
    psi4.core.clean()

def main_scf_guess(): 
    from scf_guess_tools import Backend, load, calculate
    import psi4, pyscf
    from pyscf import gto
    pyscf.lib.num_threads(48)

    _, e_ref = read_molecule("./064590_valid.xyz")
    # pyscf backend
    backend = Backend.PY
    mol = load("./064590_valid.xyz", backend, symmetry = False)
    wf = calculate(molecule=mol,
                   basis = "6-31G(2df,p)", # custom_basis_py.gbs
                   guess = None,
                   method = "DFT",
                   functional = "WB97X_V",
                   cache = "./cache")
    scf_energy_pyscf = wf.electronic_energy() + wf.nuclear_repulsion_energy()
    print(f"Final energy: {scf_energy_pyscf} Ha compared to the reference energy of {e_ref} (delta: {e_ref - scf_energy_pyscf})")

    return 

    # pure pyscf
    mol = pyscf.gto.Mole()
    mol.fromfile("./064590_valid.xyz")
    # with open("./custom_basis_py.gbs", "r") as f:
    with open("./6-31G_2df_p.gbs", "r") as f:
        basis_str = f.read()
    mol.basis = gto.basis.parse_nwchem.parse(basis_str)
    # didn't work out :((
#     mol.basis = gto.parse("""
# H    S
#      18.7311370              0.03349460
#       2.8253937              0.23472695
#       0.6401217              0.81375733
# H    S
#       0.1612778              1.00000000
# H    P
#       1.5000000              1.0000000
# # NOTE >>>> second P missing in custom format
# #BASIS SET: (10s,4p,2d,1f) -> [3s,2p,2d,1f]
# C    S
#    3047.5249000              0.0018347
#     457.3695100              0.0140373
#     103.9486900              0.0688426
#      29.2101550              0.2321844
#       9.2866630              0.4679413
#       3.1639270              0.3623120
# C    SP
#       7.8682724             -0.1193324              0.0689991
#       1.8812885             -0.1608542              0.3164240
#       0.5442493              1.1434564              0.7443083
# C    SP
#       0.1687144              1.0000000              1.0000000
# C    D
#       1.6000000              1.0000000
# C    D
#       0.4000000              1.0000000
# C    F
#       0.8000000              1.0000000
# # >>>>>First and second D differs#
# #BASIS SET: (10s,4p,2d,1f) -> [3s,2p,2d,1f]
# O    S
#    5484.6717000              0.0018311
#     825.2349500              0.0139501
#     188.0469600              0.0684451
#      52.9645000              0.2327143
#      16.8975700              0.4701930
#       5.7996353              0.3585209
# O    SP
#      15.5396160             -0.1107775              0.0708743
#       3.5999336             -0.1480263              0.3397528
#       1.0137618              1.1307670              0.7271586
# O    SP
#       0.2700058              1.0000000              1.0000000
# O    D
#       1.6000000              1.0000000
# O    D
#       0.4000000              1.0000000
# O    F
#       1.4000000              1.0000000
#                           """
#                           )
    
    
    mol.build()
    mf = pyscf.scf.RKS(mol)
    mf.xc = "B3LYPG"
    energy = mf.kernel()
    scf_energy_pyscf = energy + mol.energy_nuc()
    print(f"Final energy: {scf_energy_pyscf} Ha compared to the reference energy of {e_ref} (delta: {e_ref - scf_energy_pyscf})")

if __name__ == "__main__":
    # main()
    main_scf_guess()